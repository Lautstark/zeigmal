...
| **druckwerk** (archived) | printable material; a *targets* seam of code carrier + audio bundle, with "QR/NFC" named as a future target | **referenced**: a druckwerk NFC target would have emitted printed card faces plus a card map; zeigmal has neither, the record is on the sticker, but the seam's rule that an export file never holds provider pixels binds here too |
...
For the MVP: nowhere but the phone. The card's record is written on the phone,
in the adult mode, with the card shown as SIGNdigital serves it; the video is
fetched from SIGNdigital when the card comes in. There is no Kartensatz, no
manifest, no card map, and no file to prepare.

What the family will still be asked for, later:

- **stimmquelle**, for a spoken word on cards whose clip is silent; the
  fingerprint fields (`voice`, `text`, `key`) are what a future `voice=1` field
  on the sticker would point at.
- **A browser tool on GitHub Pages** for anything that is not NFC: choosing
  videos from a second provider, recording own clips, previewing a card the way
  the phone shows it. A desktop browser cannot reach a USB NFC reader on macOS
  (PC/SC claims the device before WebUSB can), so writing stickers stays on the
  phone whatever else moves to the browser.
