...
class PresenceTest {
    ...
    private fun seen(t: TagId) = presence.seen(TagEvent.Seen(t, null, listOf("NfcA")))
    ...
    fun `a resting card that the reader keeps losing is one card`() {
        ...
        assertEquals(listOf<TagEvent>(TagEvent.Seen(a, null, listOf("NfcA"))), events)
    ...
    fun `gone arrives once the hold has passed without a new sighting`() {
        ...
        fire()
        assertEquals(listOf<TagEvent>(TagEvent.Seen(a, null, listOf("NfcA")), TagEvent.Gone(a)), events)
        seen(a)
    ...
    fun `another card ends the first at once`() {
        ...
        seen(b)
        assertEquals(
            listOf<TagEvent>(TagEvent.Seen(a, null, listOf("NfcA")), TagEvent.Gone(a), TagEvent.Seen(b, null, listOf("NfcA"))),
            events,
        )
        fire()
        assertEquals(3, events.size)
